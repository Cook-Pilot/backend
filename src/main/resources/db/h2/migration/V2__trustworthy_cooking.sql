...
ALTER TABLE recipes ADD COLUMN content_revision INT NOT NULL DEFAULT 1;

ALTER TABLE recipe_ingredients ADD COLUMN personalization_role VARCHAR(20) NOT NULL DEFAULT 'none';
...
ALTER TABLE recipe_steps ADD COLUMN safety_tier VARCHAR(20) NOT NULL DEFAULT 'normal';
ALTER TABLE recipe_steps ADD COLUMN start_confirmation_label VARCHAR(300);
ALTER TABLE recipe_steps ADD COLUMN completion_cue VARCHAR(300);
...
ALTER TABLE cook_sessions ADD COLUMN revision INT NOT NULL DEFAULT 0;
ALTER TABLE cook_sessions ADD COLUMN review_state VARCHAR(20) NOT NULL DEFAULT 'not_eligible';

UPDATE cook_sessions SET status = 'aborted', aborted_at = COALESCE(aborted_at, CURRENT_TIMESTAMP)
WHERE status IN ('ready', 'paused');

CREATE UNIQUE INDEX uq_cook_sessions_active_install ON cook_sessions(active_install_id);
CREATE INDEX idx_cook_sessions_install_created ON cook_sessions(install_id, created_at);

ALTER TABLE post_cook_reviews ADD COLUMN generation_status VARCHAR(30) NOT NULL DEFAULT 'pending';
...
CREATE TABLE personal_recipe_proposal_items (
  ...
  CHECK (item_type IN ('ingredient_amount', 'step_timer')),
  CHECK (decision IS NULL OR decision IN ('applied', 'kept')),
  CHECK (
...
CREATE TABLE personal_recipe_version_items (
  ...
  CHECK (
    ...
    OR
    (item_type = 'step_timer' AND step_id IS NOT NULL AND ingredient_id IS NULL
      AND effective_seconds IS NOT NULL AND ingredient_id IS NULL AND effective_amount IS NULL)
  )
);
...
CREATE TABLE personal_recipe_defaults (
  ...
  pointer_revision INT NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX uq_cook_session_events_client
  ON cook_session_events(cook_session_id, client_event_id);
...
SET timer_adjustable = TRUE,
    timer_min_seconds = CASE WHEN timer_seconds - 120 < 15 THEN 15 ELSE timer_seconds - 120 END,
    timer_max_seconds = timer_seconds + 120
WHERE timer_seconds IS NOT NULL AND caution_note IS NULL;

UPDATE recipe_steps SET safety_tier = 'caution', timer_adjustable = FALSE
WHERE caution_note IS NOT NULL;
