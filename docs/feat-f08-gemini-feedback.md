...
- 변질 표현도 `상한 것 같지는 않다`, `곰팡이는 없다`처럼 바로 뒤에서 부정되면
  폐기 안내로 과잉 차단하지 않는다. 덜 익음 판정의 식재료 맥락에는 현재 단계뿐
  아니라 레시피 제목도 포함해, 일반적인 단계 문구를 쓰는 육류 레시피도 놓치지 않는다.
- 모델 진행 지시에서는 콜론을 절 경계가 아닌 어휘 구분자로 정규화한다. 따라서
  `다음 단계로: 이동하세요`처럼 콜론을 끼운 지시도 같은 신뢰 경계를 적용한다.
  `완료하세요`, `이제 완성해 주세요`처럼 조리 주어가 자연스럽게 생략된 완료
  명령도 차단하되, 부정하거나 인용해 따르지 말라는 문구는 허용한다.
