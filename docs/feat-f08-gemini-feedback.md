...
- 알레르기와 덜 익음 발화는 절 순서대로 상태를 갱신한다. 과거 사건·부정·현재
  ...
  경계로 본다.
- 변질 표현도 `상한 것 같지는 않다`, `곰팡이는 없다`처럼 바로 뒤에서 부정되면
  폐기 안내로 과잉 차단하지 않는다. 앞 절의 보고를 뒤 절에서 `아니`/`정정하면`/
  `사실은`으로 명시적으로 바로잡으면 최신 정정을 따르되, 다른 변질의 부정은 기존
  위험을 지우지 않고 정정 뒤 새로 보고된 변질은 다시 현재 위험으로 남긴다.
- 덜 익음 판정의 식재료 맥락에는 현재 단계뿐 아니라 레시피 제목도 포함한다.
  `제육볶음`과 명시적인 육류·생선 이름은 보강하되, `고기 없는`/`고기 없이`와
  비건·채식 제목은 제목만으로 육류 맥락을 만들지 않는다.
- 모델 진행 지시에서는 콜론을 절 경계가 아닌 어휘 구분자로 정규화한다. 따라서
  `다음 단계로: 이동하세요`처럼 콜론을 끼운 지시도 같은 신뢰 경계를 적용한다.
  `완료하세요`, `이제 완성해 주세요`, `완료됐습니다`, `완료해야 합니다`처럼
  조리 주어가 자연스럽게 생략된 완료 명령·선언도 차단하되, 부정하거나 인용해
  따르지 않거나 쓰지 말라는 문구는 허용한다.
