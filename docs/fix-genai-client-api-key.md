...
| `GoogleGenAiClientConfig` | 호출 상한 8초 → **6초**. 아래 참고 |

### 왜 8초가 아니라 6초인가

프론트도 8초에 끊고 "네트워크가 안 좋다"고 말한다. 서버 상한이 같은 8초면 목 폴백이 **사용자에게
도달할 수 있는 창이 0** 이다 — 8초에 끊어 목을 만들어 보내봐야 프론트는 이미 포기한 뒤고, 진짜
답이 7.9초에 와도 마찬가지다. `AiFeedbackService` 가 목 폴백을 들고 있는 이유 자체가 무력화된다.

이 버그가 고쳐지기 전에는 이 값이 한 번도 발동한 적이 없다 — 잘못된 키라 Gemini 가 400을 즉시
돌려줬기 때문이다. 실제 호출이 되기 시작한 지금부터 의미를 갖는다.

실측(실제 키, 연속 5회): `5172 / 3585 / 2911 / 3181 / 2858 ms`. 최대 5.17초. 6초면 정상 응답은
통과하고, 넘어가는 경우엔 목이 ~6초에 나가 프론트 마감 안에 들어간다. 샘플 5개라 얇다 —
운영 지연이 쌓이면 재검토할 값이다.
