...
- 컬렉션을 **보냈다면 원본 행 전체를 커버해야 한다**: 각 원본 재료·단계는 항목으로 존재하거나
  `omitted=true`로 명시해야 하고, 누락되면 400으로 거부한다. 목록 부재를 암묵적 생략(REMOVE)으로
  해석하지 않는다 — 부분 페이로드가 조용히 "재료 대량 삭제" 버전을 만드는 사고를 막기 위함이다.
- 한쪽 컬렉션만 보내면 보낸 쪽만 판정하고, 보내지 않은 쪽은 "변경 없음"으로 처리한다.
