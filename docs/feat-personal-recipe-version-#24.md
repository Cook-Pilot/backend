...
- `ingredients`/`steps`를 **둘 다 보내지 않으면** 후기만 저장하고 버전을 만들지 않는다.
  개인 버전을 선택해 조리했어도 마찬가지다(별점·메모만 전송 시 빈 버전이 생기지 않는다).
- 컬렉션을 **보냈다면 활성 원본 행 전체를 커버해야 한다**: 각 원본 재료·단계는 항목으로 존재하거나
  `omitted=true`로 명시해야 하고, 누락되면 400으로 거부한다. 단, 선택한 개인 버전에 명시적인
  `REMOVE`가 있어 합성 결과에서 이미 사라진 원본만 해당 source diff로 복원한다. 그 밖의 목록
  부재를 암묵적 생략(REMOVE)으로 해석하지 않는다 — 부분 페이로드가 조용히 "재료 대량 삭제"
  버전을 만드는 사고를 막기 위함이다.
- 한쪽 컬렉션만 보내면 보낸 쪽만 판정한다. 개인 버전을 선택했다면 보내지 않은 쪽은 source의
  누적 diff를 유지하고, 원본을 선택했다면 "변경 없음"으로 처리한다.
- 항목의 `omitted` 필드를 생략하면 `false`(실행함)로 처리한다.
...
제공하지 않는다. 재료·단계를 실행하지 않았다면 `omitted=true`로 생략을 표현한다
(단계에도 `omitted` 필드가 있으며, 목록에서 빼는 방식의 암묵적 생략은 400이다).
...
- 원본 재료를 누락한 부분 스냅샷 400 거부
- source에서 제거되지 않은 원본 단계 누락 400 거부
- source의 명시적인 REMOVE는 합성 스냅샷에서 빠져도 정상 복원
- 한쪽 스냅샷만 전송해도 반대쪽 source diff 보존
- 제거된 단계 뒤 ADD의 위치와 여러 ADD의 순서 보존
- 스냅샷 없이 개인 버전만 선택하면 후기만 저장(빈 버전 미생성)
- 단계 `omitted=true` 생략 시 REMOVE 버전 생성
- 최근 개인 버전 5개 제한
- 기간별 조리 이력 조회
