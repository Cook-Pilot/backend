...
2. 서버는 사용자 행을 잠근 뒤 `(user_id, client_session_id)`로 이미 저장된 조리인지 확인한다.
...
MVP에서는 기존 재료의 양·단위와 기존 타이머·주의 문구를 완전히 삭제하는 편집은
제공하지 않는다. 재료 자체를 사용하지 않았다면 `omitted=true`로 생략을 표현한다.
...
홈의 최근 조리도 서버 저장 시각이 아닌 실제 조리 시각 `cookedAt`을 기준으로
레시피별 최신 기록을 선택하고 정렬한다.
...
- 동일 조리 세션의 순차·동시 재전송 시 같은 결과 반환
- 실제 조리 시각 기준 최근 조리 선택·정렬
- 기존 재료의 양·단위 제거 요청 거부
- 최근 개인 버전 5개 제한
- 기간별 조리 이력 조회
