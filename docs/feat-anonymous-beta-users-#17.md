...
- 유효한 UUID: 해당 사용자로 처리
- 형식이 잘못된 UUID: 400
- 존재하지 않는 UUID: 404와 `USER_NOT_FOUND` 오류 코드
- 헤더 없음: 401과 `USER_SESSION_REQUIRED` 오류 코드
...
- 헤더로 현재 사용자 재조회
- 잘못된 UUID 400
- 존재하지 않는 사용자 404와 구조화된 오류 코드
- 사용자 헤더가 없는 개인화 요청 401
- 같은 설치 UUID의 순차·동시 생성 요청에 동일 사용자 반환
- 서로 다른 사용자 간 즐겨찾기 분리
- 서로 다른 사용자 간 최근 조리·후기·개인 레시피 목록/상세 분리
...
개인화 API는 사용자 헤더가 없을 때 공용 데모 사용자로 대체하지 않는다.
...
생성은 PostgreSQL의 `ON CONFLICT DO NOTHING`으로 처리한다. 조회 후 INSERT하는
방식과 달리 같은 설치 UUID의 동시 요청도 고유 인덱스 안에서 원자적으로 처리된다.
